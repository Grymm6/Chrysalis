...
namespace Chrysalis
{
	class CCharacterAnimationComponent
	...
	public:
		struct SDefaultScopeSettings
		{
			...
			{
				desc.SetGUID("{A3A8129E-D6E2-43AE-8DFA-D517D854B107}"_cry_guid);
				desc.SetLabel("Character Animation Component Default Scope");
		...
		CCharacterAnimationComponent() = default;
		virtual ~CCharacterAnimationComponent();

		static void ReflectType(Schematyc::CTypeDesc<CCharacterAnimationComponent>& desc)
		{
			desc.SetGUID("{DDBD2129-A579-466F-96A1-7B378A35CAB3}"_cry_guid);
			desc.SetEditorCategory("Geometry");
			desc.SetLabel("Character Animation");
			desc.SetDescription("Exposes playback of character animation using Mannequin.");
			desc.SetIcon("icons:General/Mannequin.ico");
			desc.SetComponentFlags({ IEntityComponent::EFlags::Transform, IEntityComponent::EFlags::Socket, IEntityComponent::EFlags::Attach });

			desc.AddMember(&CCharacterAnimationComponent::m_type, 'type', "Type", "Type", "Determines the behavior of the static mesh", Cry::DefaultComponents::EMeshType::RenderAndCollider);
			desc.AddMember(&CCharacterAnimationComponent::m_characterFile, 'file', "Character", "Character", "Determines the character to load", "");
			desc.AddMember(&CCharacterAnimationComponent::m_renderParameters, 'rend', "Render", "Rendering Settings", "Settings for the rendered representation of the component", Cry::DefaultComponents::SRenderParameters());

			desc.AddMember(&CCharacterAnimationComponent::m_databasePath, 'dbpa', "DatabasePath", "Animation Database", "Path to the Mannequin .adb file", "");
			desc.AddMember(&CCharacterAnimationComponent::m_defaultScopeSettings, 'defs', "DefaultScope", "Default Scope Context Name", "Default Mannequin scope settings", CCharacterAnimationComponent::SDefaultScopeSettings());
			desc.AddMember(&CCharacterAnimationComponent::m_bAnimationDrivenMotion, 'andr', "AnimDriven", "Animation Driven Motion", "Whether or not to use root motion in the animations", true);
			desc.AddMember(&CCharacterAnimationComponent::m_bGroundAlignment, 'grou', "GroundAlign", "Use Ground Alignment", "Enables adjustment of leg positions to align to the ground surface", false);

			desc.AddMember(&CCharacterAnimationComponent::m_physics, 'phys', "Physics", "Physics", "Physical properties for the object, only used if a simple physics or character controller is applied to the entity.", Cry::DefaultComponents::SPhysicsParameters());
		}
